...
State changes: creates the entry PDA (rent funded by the backend; the `init` constraint blocks duplicates with no extra logic needed); increments `campaign.entry_count` by one. **No authority over the prize vault at all** — that separation is intentional, so a bug in the join path can't reach prize funds. Guards: `campaign.state == open`, clock before `end_time`, signer must equal `campaign.verifier`.
...
Deliberately **not** backend-signed — designed as a permissionless "crank" callable by anyone once `end_time` passes, unlike `join_campaign`. Randomness source is leaning toward a VRF (Switchboard or ORAO) rather than a recent slot hash: slot hashes are influenceable by validators, which would undermine the one claim the whole product rests on.
